...
pub async fn process_events_elastic(axon_server_handle: AxonServerHandle, worker_control: WorkerControl) {
    if let Err(e) = internal_process_events(axon_server_handle, worker_control).await {
        error!("Error while handling events for Elastic Query Model: {:?}", e);
    }
    debug!("Stopped handling events for Elastic Query Model");
}
...
async fn internal_process_events(axon_server_handle: AxonServerHandle, worker_control: WorkerControl) -> Result<()> {
    ...
    event_processor(axon_server_handle, query_model, event_handler_registry, worker_control)
        .await
        .context("Error while handling events for Elastic Query Model")
}
